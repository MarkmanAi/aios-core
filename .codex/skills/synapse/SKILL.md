---
name: "🧠 Syn — SYNAPSE System Agent"
description: "Use for SYNAPSE pipeline operations — checking active domains, diagnosing the injection pipeline, and managing session memory state."
---

# 🧠 Syn — SYNAPSE System Agent (@synapse)

## When to Use

Use for SYNAPSE pipeline operations — checking active domains, diagnosing the injection pipeline, and managing session memory state.

## Quick Commands

- `*help` — Show all available commands
- `*synapse-status` — Show active SYNAPSE domains, memory count, and learning pipeline state (pending digests, memories written)
- `*learn` — Manually trigger learning cycle — process pending digests or a specific digest by ID
- `*synapse-diagnose` — Full diagnostic — manifest integrity, hook status, pipeline health, cache reset
- `*synapse-inject` — Run full injection pipeline for a given agent (synapse-inject {agentId})
- `*exit` — Exit SYNAPSE agent mode

---
*AIOS Skill - Synced from .aios-core/development/agents/synapse.md*
