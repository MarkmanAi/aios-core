 ...
 * Target: >= 90% coverage
 *
 * @module core/synapse/memory/__tests__/memory-loader.test
 */

const fs = require('fs').promises;
const path = require('path');
const { MemoryLoader, createMemoryLoader, AGENT_SECTOR_PREFERENCES } = require('../../../../.aios-core/core/synapse/memory/memory-loader');
...
describe('MemoryLoader', () => {
  ...
  describe('loadForAgent', () => {
    it('should load HOT tier memories (Layer 1) by default', async () => {
      const result = await loader.loadForAgent('dev');

      expect(result.memories).toBeDefined();
      expect(Array.isArray(result.memories)).toBe(true);
      expect(result.metadata.agent).toBe('dev');
      expect(result.metadata.budget).toBe(2000);
    });

    it('should include WARM tier (Layer 2) when budget allows', async () => {
      const result = await loader.loadForAgent('dev', { budget: 2000, layers: [1, 2] });

      expect(result.memories).toBeDefined();
      expect(result.metadata.tiers).toContain('hot');
      expect(result.metadata.tiers).toContain('warm');
    });

    it('should skip WARM tier when layers excludes layer 2', async () => {
      const result = await loader.loadForAgent('dev', { budget: 2000, layers: [1] });
    ...
    it('should return empty memories on error (graceful degradation)', async () => {
      // Force retriever to throw so the catch block in loadForAgent is covered
      loader.retriever.retrieve = async () => { throw new Error('forced retriever error'); };
  ...
  describe('edge cases and error handling', () => {
    ...
    it('should handle retriever throwing in queryMemories catch block', async () => {
      // Force retriever to throw so the catch block in queryMemories is covered
      loader.retriever.retrieve = async () => { throw new Error('forced retriever error'); };
      loader.retriever.queryMemories = async () => { throw new Error('forced retriever error'); };

      const memories = await loader.queryMemories('dev', { layer: 1 });

      expect(memories).toEqual([]);
    });

    it('should handle retriever throwing in getMemoryById catch block', async () => {
      // Force retriever to throw so the catch block in getMemoryById is covered
      loader.retriever.getMemoryById = async () => { throw new Error('forced retriever error'); };

      const memory = await loader.getMemoryById('some-id');
