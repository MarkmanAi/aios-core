...
describe('GreenfieldHandler', () => {
  ...
  describe('isGreenfield (AC1)', () => {
    ...
    test('should return false when docs/ exists', () => {
      fs.existsSync.mockImplementation((p) => {
        // Handle both Unix (docs/) and Windows (docs\ or docs) paths
        return p.endsWith('docs/') || p.endsWith('docs\\') || p.endsWith('docs');
      });
      expect(handler.isGreenfield()).toBe(false);
    });
    ...
    test('should accept custom project path', () => {
      ...
      // Check that the path contains 'custom' and 'path' (platform-independent)
