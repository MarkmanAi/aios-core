 ...
 * Target: >= 90% coverage
 *
 * @module pro/memory/__tests__/memory-index.test
 */
...
`,
    ...
    it('should search by date', async () => {
      const result = await indexManager.search({ date: '2026-02-09' });

      expect(result.results.length).toBe(2);
      expect(result.results.every(r => r.timestamp.startsWith('2026-02-09'))).toBe(true);
    });
