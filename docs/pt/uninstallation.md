...
Este guia fornece instruções completas para desinstalar o MarkmanAi AIOS do seu sistema.
...
**Aviso**: Desinstalar o MarkmanAi AIOS irá:
...
A forma mais rápida de desinstalar o MarkmanAi AIOS:
...
```bash
#!/bin/bash
echo "Desinstalação Completa do MarkmanAi AIOS"
echo "================================="

# Confirmação
read -p "Isso removerá TODOS os dados do MarkmanAi AIOS. Continuar? (y/N) " -n 1 -r
echo
if [[ ! $REPLY =~ ^[Yy]$ ]]; then
    exit 1
fi
...
Write-Host "Limpando MarkmanAi AIOS do Registro do Windows..."
...
#!/bin/bash
# force-uninstall.sh
echo "Desinstalação forçada do MarkmanAi AIOS..."
...
Se você quiser reinstalar o MarkmanAi AIOS:
...
1. **Consulte a Documentação**
   ...
   - [Resolução de Problemas](https://github.com/MarkmanAi/aios-core/wiki/troubleshooting)
