...
O MarkmanAi AIOS pode ser instalado via NPX para uma configuração rápida sem instalação global. Este guia cobre o uso correto e a resolução de problemas para instalações baseadas em NPX.

## Início Rápido

### Uso Correto

Sempre execute `npx @markmanai/aios-core install` **a partir do diretório do seu projeto**:
...
O NPX executa pacotes em **diretórios temporários** (ex: `/private/var/folders/.../npx-xxx/` no macOS). Quando o MarkmanAi AIOS é executado a partir desses locais temporários, ele não consegue:
...
A partir da versão 4.31.1, o MarkmanAi AIOS detecta automaticamente quando está sendo executado a partir de um diretório temporário do NPX e exibe uma mensagem de erro útil:
...
Veja: https://@markmanai/aios-core.dev/docs/npx-install
```
...
O MarkmanAi AIOS detecta esses padrões e previne instalação incorreta.
...
O MarkmanAi AIOS v4.31.1+ implementa detecção em duas camadas:
...
2. **Camada SECUNDÁRIA** (`tools/installer/bin/aios.js`):
...
- Documentação: https://@markmanai/aios-core.dev/docs
