...
Este documento lista todas as variáveis de ambiente usadas pelo MarkmanAi AIOS e seus componentes.

## Visão Geral

O MarkmanAi AIOS usa variáveis de ambiente para configuração, chaves de API e informações sensíveis. **Nunca faça commit de variáveis de ambiente no repositório.**

## Variáveis de Ambiente Obrigatórias

### Core do Framework

Atualmente, o MarkmanAi AIOS não requer nenhuma variável de ambiente obrigatória para operação básica. Toda a configuração é feita através do `core-config.yaml` e arquivos de configuração dos Squads.
...
O MarkmanAi AIOS usa `dotenv` (se instalado) ou o suporte nativo a variáveis de ambiente do Node.js. As variáveis de ambiente são carregadas automaticamente dos arquivos `.env` na raiz do projeto.
