...
API REST para sincronizar squads con MarkmanAi y descubrir squads del marketplace.

## Vision General

La API de Squads permite:

- **Sync**: Enviar squads locales a la nube de MarkmanAi
...
Obtiene tu clave API de: https://markmanai.com/settings/api-keys

## Endpoints

### Sincronizar Squad

Envia una definicion de squad a MarkmanAi.
...
```bash
curl -X PATCH https://api.markmanai.com/api/squads/mi-squad \
  -H "Authorization: Bearer sk_tu_clave_api" \
  -H "Content-Type: application/json" \
  -d '{"isPublic": true}'
```
...
```bash
curl -X DELETE https://api.markmanai.com/api/squads/mi-squad-antiguo \
  -H "Authorization: Bearer sk_tu_clave_api"
```
...
El comando `*sync-squad-markmanai` usa esta API:

```bash
# Sincronizar squad individual
@squad-creator
*sync-squad-markmanai ./squads/mi-squad --public
...
```bash
export MARKMANAI_API_TOKEN="sk_tu_clave_api"
```
...
{
  "info": {
    "name": "API de Squads MarkmanAi",
    "description": "API REST para el Marketplace de Squads de MarkmanAi",
...
Guarda el JSON anterior como `markmanai-squads-api.postman_collection.json` e importa en Postman.
