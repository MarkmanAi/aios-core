...
This guide provides comprehensive instructions for uninstalling MarkmanAi AIOS from your system.
...
⚠️ **Warning**: Uninstalling MarkmanAi AIOS will:
...
The fastest way to uninstall MarkmanAi AIOS:
...
Remove-Item -Path "HKCU:\Software\MarkmanAi AIOS" -Recurse
```
...
```bash
#!/bin/bash
echo "MarkmanAi AIOS Complete Uninstall"
echo "================================="

# Confirmation
read -p "This will remove ALL MarkmanAi AIOS data. Continue? (y/N) " -n 1 -r
echo
if [[ ! $REPLY =~ ^[Yy]$ ]]; then
    exit 1
fi
...
Write-Host "Cleaning MarkmanAi AIOS from Windows Registry..."
...
#!/bin/bash
# force-uninstall.sh
echo "Force uninstalling MarkmanAi AIOS..."
...
```bash
# Remove AIOS-specific git hooks
rm -f .git/hooks/*aios*
...
# Commit removal
git add -A
git commit -m "Remove MarkmanAi AIOS"
```
...
If you want to reinstall MarkmanAi AIOS:
...
1. **Check Documentation**
   - [FAQ](https://github.com/MarkmanAi/aios-core/wiki/faq#uninstall)
   - [Troubleshooting](https://github.com/MarkmanAi/aios-core/wiki/troubleshooting)
