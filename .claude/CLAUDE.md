...
O MarkmanAi AIOS possui uma **Constitution formal** com princípios inegociáveis e gates automáticos.
...
**Constitution v1.1.0** (amended 2026-03-19 — Article VII: Runtime Governance added). **Gates automáticos bloqueiam violações.** Consulte a Constitution para detalhes completos.

---

## Premissa Arquitetural: CLI First

O MarkmanAi AIOS segue uma hierarquia clara de prioridades que deve guiar **TODAS** as decisões:
...
Codex CLI is the second execution runtime. Same Constitution applies (Art. VII).

| Runtime | Config | Instruction File |
|---------|--------|-----------------|
| Claude Code | `.claude/` | `.claude/CLAUDE.md` |
| Codex CLI | `.codex/` | `AGENTS.md` (root) |

**Concurrency rule:** Only one runtime actively edits files at a time. If Codex is implementing, Claude operates in read/review mode (and vice versa).

**Protected paths — do not modify:**
- `.codex/config.toml` — governance artifact managed by the Constitution
- `.codex/rules/` — command guardrails (governance artifact)
- `.codex/skills/` — generated by IDE Sync, do not edit directly

> **Cross-reference:** Constitution Article II — Agent Authority (NON-NEGOTIABLE) | Article VII — Runtime Governance (MUST)
