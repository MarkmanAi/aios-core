...
function cmdNext(slug, options = {}) {
  ...
  if (nextPhase.human_checkpoint) {
    const _updateResult = runStateManager(`update ${slug} --phase ${nextPhase.id} --status checkpoint`);
    outputJson({
      success: true,
      action: 'next',
      slug,
  ...
  const _updateResult = runStateManager(`update ${slug} --phase ${nextPhase.id} --status in_progress`);
